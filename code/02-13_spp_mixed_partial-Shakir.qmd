...
# Introduction  - Test from new lab desktop again again
...
options(contrasts = c("contr.sum", "contr.poly")) # Does not have any effect on means, put has an effect on the ANOVA model and its p-value

# Model fitting
spp_rcbd_mix_mod <- lme4::lmer(yield_kgha ~ nrate_kgha*krate_kgha + 
                                 (1|rep/krate_kgha), #To nest krate_kgha inside rep; we mention the  largest level of replication i.e., block, then the whole plot factor i.e., lower level random effect factor "krate_kgha" is followed by a forward slash (/)
                               data = spp_rcbd_dfw)

#For split-split plot design
#spp_rcbd_mix_mod <- lme4::lmer(yield_kgha ~ nrate_kgha*krate_kgha*prate_kgha + 
                                 #(1|rep/krate_kgha/nrate_kgha), #To nest krate_kgha inside rep; we mention the  largest level of replication i.e., block, then the whole plot factor i.e., lower level random effect factor "krate_kgha" is followed by a forward slash (/)
                               #data = spp_rcbd_dfw)

# (1|rep/krate_kgha) = (1|rep) + (1|krate_kgha)
...
Very important note: We extract the means based on what effects are significant in the ANOVA table
...
```{r randeff_block}
randeff_block <- ranef(spp_rcbd_mix_mod)$rep #to output only the values for "rep"
...
#randeff_blockkrate <- ranef(spp_rcbd_mix_mod) #run this first to copy the `krate_kgha:rep` mentioned after the $ sign in the next line
...
- **Variance homoscedastic**: seems constant across the x-axis. (i.e., no fin shape).
...
- **Normality**: residuals seem normal (as per the density plot).  
...
For nrate_kgha:krate_kgha: SE = 330, df = 22.9
For nrate_kgha,SE = 209, df = 8.19
For krate_kgha, SE = 232, df = 8.28
...
Points to notice:  
  ...
  - The **SE of the SP treatment factor will always be lower than that from the WP treatment factor**. Why?  Because how many - Listen 1:01:00
